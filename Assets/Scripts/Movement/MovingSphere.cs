    ...
    public class MovingSphere : MonoBehaviour
    {
        ...
        private void UpdateBall()
        {
            ...
            else if (Swimming)
            {
                ...
                rotationFactor = m_ballSwimRotation;
            }
            else if (!OnGround)
            {
                if (OnSteep)
                {
                    rotationPlaneNormal = m_lastSteepNormal;
                }
                else
                {
                    rotationFactor = m_ballAirRotation;
                }
            }
            ...
            // 根据小球的位移大小和半径旋转小球
            Vector3 movement = (m_rigidbody.velocity - m_lastConnectionVelocity) * Time.deltaTime;
            movement -= rotationPlaneNormal * Vector3.Dot(movement, rotationPlaneNormal);
            float distance = movement.magnitude;

            // 当小球跟随连接物体旋转时，给小球施加角速度
            Quaternion rotation = m_ball.localRotation;
            if (m_connectedBody && m_connectedBody == m_previousConnectedBody)
            {
                rotation = Quaternion.Euler(m_connectedBody.angularVelocity * (Mathf.Rad2Deg * Time.deltaTime)) *
                           rotation;
                if (distance < 0.001f)
                {
        ...
        private void ClearState()
        {
            ...
            m_lastConnectionVelocity = m_connectionVelocity;
        ...
        private void AdjustVelocity()
        {
            ...
            if (Swimming)
            {
                m_velocity += m_upAxis * adjustment.y;
            }
        }
        ...
        /// 上一次接触地面时的法线
        /// </summary>
        private Vector3 m_lastContactNormal;

        /// <summary>
        /// 上一次接触峭壁时的法线
        /// </summary>
        private Vector3 m_lastSteepNormal;

        /// <summary>
        /// 上一次的连接速度
        /// </summary>
        private Vector3 m_lastConnectionVelocity;
        ...
        /// </summary>
        [SerializeField]
        private Transform m_ball = default;
        ...
        /// </summary>
        [SerializeField, Min(0f)]
        private float m_ballAlignSpeed = 180f;

        /// <summary>
        /// 小球在空中时的旋转因子
        /// </summary>
        [SerializeField, Min(0f)]
        private float m_ballAirRotation = 0.5f;

        /// <summary>
        /// 小球在游泳时的旋转因子
        /// </summary>
        [SerializeField, Min(0f)]
        private float m_ballSwimRotation = 2f;
